...
export async function POST(request: NextRequest) {
  ...
  try {
    const body = (await request.json()) as { credentialId?: string; platform?: CancelPlatform };
    ...
    const orders = await fetchCancelReadyOrders(supabase, userData.user.id, body.platform);
    if (orders.length === 0) {
      return NextResponse.json({ platform: body.platform, matched: [], skipped: [], remoteCount: 0, dryRun: isDryRun() });
    }
    ...
    return NextResponse.json({ ...preview, dryRun: isDryRun() });
  } catch (err) {
