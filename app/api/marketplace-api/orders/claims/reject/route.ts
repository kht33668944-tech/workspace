...
import { rejectCancelRequests, type SyncPlatform } from "@/lib/marketplace/order-sync";
import { isDryRun } from "@/lib/marketplace/common";

export const maxDuration = 120;

/** 구매자 취소요청 거절(발송 강행): { orderIds: string[] } — 운송장이 있어야 함 */
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (orderIds.length === 0) return NextResponse.json({ error: "거절할 주문을 선택하세요." }, { status: 400 });
    ...
    for (const platform of ["coupang", "smartstore"] as SyncPlatform[]) {
      ...
      results.push(...(await rejectCancelRequests({ supabase, userId, credentialId: cred.id, platform, orderIds: byPlatform[platform], ...clients })));
    }
    return NextResponse.json({ dryRun: isDryRun(), results });
  } catch (err) {
    const message = err instanceof Error ? err.message : String(err);
    console.error("[claims-reject] 오류:", message);
    return NextResponse.json({ error: `취소 거절 실패: ${message}` }, { status: 500 });
  }
}
