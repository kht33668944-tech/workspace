// ss-cancel.mjs 결과를 발주서에 반영한다 — 판매취소 성공한 건만 취소준비 → 취소완료.
import { serviceClient } from "./_lib.mjs";
import fs from "fs";

const done = JSON.parse(fs.readFileSync("scripts/_ss-results.json", "utf8"));
...
for (const r of done) {
  ...
  if (error) console.log(`[발주서반영] 실패 ${r.수취인명}: ${error.message}`);
  else ok++;
}
