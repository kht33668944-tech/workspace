// ESM(지마켓·옥션) 취소 전체 파이프라인.
//
//   npm run esm:cancel          → 크롬 실행 + 대조 + 확인 팝업까지만 (안전 확인용)
//   npm run esm:cancel -- --go  → 위 확인 후 전건 판매취소 + 발주서 취소완료 반영
...
if (await cdpUp()) {
  ...
  spawn(CHROME, [
    ...
    "https://www.esmplus.com/Home/v2/send-processing",
  ...
  console.log("[실행] ESM에 로그인이 안 돼 있으면 크롬 창에서 로그인한 뒤 다시 실행하세요");
}

step("발주서에서 취소준비 지마켓·옥션 주문 추출", "scripts/_esm-list.mjs");
step("ESM 발송처리 목록 수집", "scripts/_esm-collect.mjs");
step("수령인명·구매자명·상품명·수량 대조", "scripts/_esm-match.mjs");

if (!GO) {
  step("확인 팝업까지 (드라이런)", "scripts/esm-cancel.mjs", ["--dry"]);
  console.log("\n[실행] 여기까지 확인했으면 아래로 전건 판매취소한다:");
  console.log("       npm run esm:cancel -- --go");
  process.exit(0);
}

step("전건 판매취소", "scripts/esm-cancel.mjs");
step("발주서 취소완료 반영", "scripts/esm-cancel-apply.mjs");
console.log("\n[실행] 끝. 결과는 scripts/_esm-results.json");
