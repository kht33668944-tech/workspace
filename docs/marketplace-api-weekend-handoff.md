> **2026-08-30 갱신**: 아래 "쿠팡 직접 API 보류" 판단은 해제됨. 쿠팡 키를 자체개발(온리브)+IP 2개(플레이오토 52.79.173.222, 로컬 PC)로 수정해 플레이오토와 키를 공유하며, 스마트스토어 내스토어 애플리케이션("온리브 통합관리")도 발급. 구현 현황은 `DEV-ENV.md` 참고. 후보 B(스마트스토어 직접)+C(쿠팡 직접)를 동시에 진행한 상태.
