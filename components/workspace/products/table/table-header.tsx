...
function ColumnFilterDropdown({ columnKey, allProducts, columnFilters, selectedValues, onChange, onClose, sort, onSort }: {
  ...
  const crossFilteredProducts = useMemo(() => {
    ...
    if (otherFilters.length === 0) return allProducts;
  ...
  }, [allProducts, columnFilters, columnKey]);

  const unique = useMemo(() => {
    const m = new Map<string, number>();
    for (const p of crossFilteredProducts) {
      const raw = p[columnKey as keyof Product];
      const v = raw == null || raw === "" ? "(빈 값)" : String(raw);
      m.set(v, (m.get(v) || 0) + 1);
    }
    return [...m.entries()].sort((a, b) => a[0].localeCompare(b[0])).map(([value, count]) => ({ value, count }));
  }, [crossFilteredProducts, columnKey]);
