...
interface StatusData {
  ...
  matchedProducts: { id: string; product_name: string; auctionCount: number; gmarketCount: number }[];
}

export default function EsmPriceImportModal({ onClose, onImported }: Props) {
  ...
  const fetchStatus = useCallback(async () => {
    ...
    try {
      const res = await fetch("/api/esm-price-inventory/status", {
  ...
  const handleUpload = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch("/api/esm-price-inventory/import", {
        method: "POST",
        headers: {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm">
      <div className="bg-[var(--bg-card)] border border-[var(--border)] rounded-2xl w-full max-w-lg mx-4 max-h-[85vh] flex flex-col shadow-2xl">
        <div className="flex items-center justify-between px-6 py-4 border-b border-[var(--border)] shrink-0">
          <div>
            <h2 className="text-base font-semibold text-[var(--text-primary)]">옥션·지마켓 상품목록 임포트</h2>
            <p className="text-xs text-[var(--text-muted)] mt-0.5">
              ESM Plus에서 다운받은 <strong>상품목록 엑셀</strong>(.xlsx)을 업로드하세요 (가격대량수정 양식 아님)
            </p>
          </div>
                  ...
                  {showMatchedList && (
                    <div className="space-y-2 pt-1">
                      <input
                        ...
                        placeholder="상품명 검색..."
                        className="w-full px-2 py-1 text-xs bg-[var(--bg-card)] border border-[var(--border)] rounded text-[var(--text-primary)] outline-none focus:border-red-400"
                      />
                          ...
                          filteredProducts.map(p => (
                            <div key={p.id} className="flex items-center justify-between px-2 py-1.5 text-xs">
                              ...
                              <span className="shrink-0 flex gap-2">
                                <span className="text-red-400">옥 {p.auctionCount}</span>
                                <span className="text-emerald-400">지 {p.gmarketCount}</span>
                              </span>
                            </div>
                          ))
                        )}
            ...
            <>
              <div
                ...
                className={`border-2 border-dashed rounded-xl p-8 text-center cursor-pointer transition-colors ${
                  dragOver ? "border-red-400 bg-red-400/10" : "border-[var(--border)] hover:border-red-400/60"
                }`}
              >
                <input
                  ref={fileInputRef}
                ...
                />
                <FileSpreadsheet className="w-10 h-10 text-red-400 mx-auto mb-2" />
                {file ? (
                  <>
          ...
          {!result ? (
            <>
              ...
              <button
                ...
                className="px-4 py-2 text-sm bg-red-600 hover:bg-red-700 disabled:opacity-40 disabled:cursor-not-allowed text-white rounded-lg transition-colors flex items-center gap-2"
              >
            ...
            <button onClick={onClose} className="px-4 py-2 text-sm bg-red-600 hover:bg-red-700 text-white rounded-lg transition-colors">
              확인
            </button>
          )}
        </div>
