...
export default function OrderSidePanel({ order, onUpdate, onClose }: OrderSidePanelProps) {
  ...
  return (
    <>
      ...
      <div
        ref={panelRef}
        className="fixed top-0 right-0 z-50 h-full w-[420px] max-w-[90vw] bg-[#1a1a2e] border-l border-white/10 shadow-2xl flex flex-col animate-slide-in"
      >
        {/* Header */}
        <div className="flex items-center justify-between px-5 py-4 border-b border-white/10 shrink-0">
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2 mb-1">
              ...
              <span className="text-xs text-white/40 truncate">{order.recipient_name || "-"}</span>
            </div>
            <h2 className="text-sm font-medium text-white truncate" title={order.product_name || ""}>{order.product_name || "상품명 없음"}</h2>
          </div>
          <button onClick={onClose} className="p-1 text-white/40 hover:text-white shrink-0 ml-2">
            <X className="w-5 h-5" />
          </button>
        </div>
        ...
        <div className="flex-1 overflow-y-auto">
          ...
          <div className="px-5 py-3 border-b border-white/10">
            <label className="text-xs text-white/50 mb-1.5 block">배송상태 변경</label>
            <select
          ...
          <div className="px-5 py-3">
            <h3 className="text-xs font-medium text-white/50 mb-3">상담내역</h3>
            {logs.length === 0 ? (
              <p className="text-xs text-white/20 text-center py-4">상담내역이 없습니다</p>
            ) : (
              <div className="space-y-3">
                {logs.map((log, i) => (
                  <div key={i} className="relative pl-5 border-l-2 border-white/10">
                    <div className="absolute left-[-5px] top-1 w-2 h-2 rounded-full bg-blue-500" />
                    <div className="flex items-center gap-2 mb-0.5">
                      <span className="text-xs font-medium text-white/70">{log.author}</span>
                      <span className="text-xs text-white/30 flex items-center gap-1">
                        <Clock className="w-3 h-3" />
                        {new Date(log.date).toLocaleString("ko-KR", { month: "short", day: "numeric", hour: "2-digit", minute: "2-digit" })}
                      </span>
                    </div>
                    <p className="text-xs text-white/60 whitespace-pre-wrap">{log.content}</p>
                  </div>
                ))}
              </div>
            )}
        ...
        <div className="px-5 py-3 border-t border-white/10 shrink-0">
          <div className="flex items-end gap-2">
            <textarea
              ...
              rows={2}
              className="flex-1 bg-white/5 border border-white/10 rounded-lg px-3 py-2 text-sm text-white placeholder:text-white/20 outline-none focus:border-blue-500/50 resize-none"
            />
            <button
              onClick={handleAddLog}
              disabled={!logInput.trim()}
              className="p-2 bg-blue-600 hover:bg-blue-700 disabled:opacity-30 text-white rounded-lg transition-colors"
            >
