...
import { KIND_TO_KEY, RUN_STATUS_LABEL, runLabelForKind, type RunKind } from "@/lib/automation-schedule";
import { formatLogTime, formatLogDate, timeAgo } from "@/lib/log-format";
import RunDetail from "./run-detail";
import { useRunAutomation } from "./run-actions";

export default function ErrorCenter({ errorRuns, onRefetch }: {
  ...
  const retry = (r: MarketplaceSyncRun) => {
    const key = KIND_TO_KEY[(r.kind ?? "orders") as RunKind];
    if (key) void run(key, r.id);
  };
                ...
                <div className="flex items-center gap-2 px-2 py-1.5 rounded-lg hover:bg-[var(--bg-hover)]">
                  <button onClick={() => setExpanded(isOpen ? null : run.id)} className="flex items-center gap-2 flex-1 min-w-0 text-left">
                    ...
                    <span className={`text-xs px-1.5 py-0.5 rounded shrink-0 ${run.status === "failed" || run.status === "running" ? "bg-red-500/10 text-red-400" : "bg-amber-500/10 text-amber-400"}`}>
                      {RUN_STATUS_LABEL[run.status === "running" ? "stale" : run.status]}
                    </span>
                    <span className="text-xs font-medium text-[var(--text-secondary)] shrink-0">{runLabelForKind(run.kind)}</span>
                  ...
                  >
                    {busyId === run.id ? <Loader2 className="w-3 h-3 animate-spin" /> : <RotateCcw className="w-3 h-3" />}
                    재시도
                  </button>
                </div>
